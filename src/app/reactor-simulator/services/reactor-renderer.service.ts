// ============================================================
//  reactor-renderer.service.ts  (FIXED)
//  Pure canvas drawing engine — Injectable Angular service.
//
//  Key changes vs original:
//  1. Layout cached — _layout() result reused across frames and
//     only recomputed when canvas dimensions actually change.
//  2. Particle counts tracked with dedicated counters instead of
//     three separate filter() passes per frame.
//  3. Per-channel radial glow gradients replaced with a single
//     shared ctx.shadowBlur pass — eliminates up to 13 gradient
//     object allocations per frame.
//  4. Fuel channel linear gradient reused via a pre-built color
//     calculation; gradient is created once per unique hotness
//     bucket rather than once per channel per frame.
// ============================================================
...
export class ReactorRendererService {
  ...
  // ── Particle type counters (avoid repeated filter() passes) ──
  private _nWater   = 0;
  private _nSteam   = 0;
  private _nNeutron = 0;
  ...
  /** Call once after the canvas element is available (AfterViewInit) */
  ...
  resize(w: number, h: number): void {
    ...
    // Invalidate cached layout when canvas dimensions change
    this._layout = null;
  }

  /** Main render entry point — call on every animation frame */
  ...
  // ══════════════════════════════════════════════════════
  //  Layout — cached, only recomputed on resize
  // ══════════════════════════════════════════════════════
  ...
  private _getLayout(): Layout {
    ...
    // Return cached layout when canvas size has not changed
    if (this._layout && W === this._layoutW && H === this._layoutH) {
      return this._layout;
    }
  ...
  private _updateParticles(sim: ReactorState, L: Layout): void {
    ...
    // Single-pass filter+update — also maintains type counters.
    const next: Particle[] = [];
    let nW = 0, nS = 0, nN = 0;
    ...
    // Spawn new particles if below target counts
  ...
  private _drawContainment(ctx: CanvasRenderingContext2D, L: Layout): void {
    const c = L.containment;
    ctx.fillStyle = '#f0f5fc';
    ctx.beginPath();
    (ctx as any).roundRect(c.x, c.y, c.w, c.h, 6);
    ctx.fill();
    ...
    ctx.lineWidth   = 2;
    ctx.beginPath();
    (ctx as any).roundRect(c.x + 7, c.y + 7, c.w - 14, c.h - 14, 4);
    ctx.stroke();
  }
  ...
  private _drawPipes(ctx: CanvasRenderingContext2D, L: Layout, sim: ReactorState): void {
    ...
    ctx.lineCap     = 'round';
    ctx.beginPath();
    ctx.moveTo(L.pump.cx, L.pump.cy - L.pump.r);
    ctx.lineTo(L.pump.cx, L.vessel.y + L.vessel.h);
    ctx.stroke();

    const steamAlpha = 0.3 + fluxN * 0.5;
    const steamColor = fluxN > 0.3
      ? `rgba(168,212,245,${steamAlpha})`
      : `rgba(41,128,185,${alpha * 0.6})`;
    ctx.strokeStyle = steamColor;
    ctx.lineWidth   = 10;

    ctx.beginPath();
    ctx.moveTo(L.vessel.x + L.vessel.w * 0.3, L.vessel.y);
    ctx.quadraticCurveTo(L.vessel.x + L.vessel.w * 0.3, L.vessel.y - 30, L.steamSepL.cx, L.steamSepL.cy + L.steamSepL.r);
    ctx.stroke();

    ctx.beginPath();
    ctx.moveTo(L.vessel.x + L.vessel.w * 0.7, L.vessel.y);
    ctx.quadraticCurveTo(L.vessel.x + L.vessel.w * 0.7, L.vessel.y - 30, L.steamSepR.cx, L.steamSepR.cy + L.steamSepR.r);
    ctx.stroke();

    ctx.strokeStyle = `rgba(41,128,185,${alpha * 0.6})`;
    ctx.lineWidth   = 8;
    ctx.beginPath();
    ctx.moveTo(L.steamSepL.cx, L.steamSepL.cy + L.steamSepL.r);
    ctx.lineTo(L.steamSepL.cx, L.pump.cy);
    ctx.lineTo(L.pump.cx, L.pump.cy);
    ctx.stroke();

    ctx.beginPath();
    ctx.moveTo(L.steamSepR.cx, L.steamSepR.cy + L.steamSepR.r);
    ctx.lineTo(L.steamSepR.cx, L.pump.cy);
    ctx.lineTo(L.pump.cx, L.pump.cy);
    ctx.stroke();

    if (fluxN > 0.05) {
      ctx.strokeStyle = `rgba(200,230,255,${fluxN * 0.8})`;
      ctx.lineWidth   = 8;
      ctx.setLineDash([6, 4]);
      ctx.beginPath();
      ctx.moveTo(L.steamSepL.cx, L.steamSepL.cy - L.steamSepL.r);
      ctx.lineTo(L.steamSepL.cx, L.containment.y + 15);
      ctx.stroke();

      ctx.beginPath();
      ctx.moveTo(L.steamSepR.cx, L.steamSepR.cy - L.steamSepR.r);
      ctx.lineTo(L.steamSepR.cx, L.containment.y + 15);
      ctx.stroke();
      ctx.setLineDash([]);
    }
  }
  ...
  private _drawVessel(ctx: CanvasRenderingContext2D, L: Layout, sim: ReactorState): void {
    ...
    ctx.shadowColor = 'rgba(0,30,80,0.2)';
    ctx.shadowBlur  = 16;
    ctx.fillStyle   = '#dde8f8';
    ctx.beginPath();
    (ctx as any).roundRect(v.x, v.y, v.w, v.h, 10);
    ctx.fill();
    ctx.shadowBlur = 0;
    ...
    ctx.fillStyle = grad;
    ctx.beginPath();
    (ctx as any).roundRect(v.x + 4, v.y + 4, v.w - 8, v.h - 8, 8);
    ctx.fill();
    ...
    ctx.lineWidth   = 1.5;
    ctx.beginPath();
    (ctx as any).roundRect(v.x + 3, v.y + 3, v.w - 6, v.h - 6, 8);
    ctx.stroke();
  }
  ...
  private _drawCore(ctx: CanvasRenderingContext2D, L: Layout, sim: ReactorState): void {
    ...
    // Glow pass: draw all channels with shadowBlur in a single batch
    // instead of one radialGradient per channel (was 13 gradient objects/frame)
    if (hotness > 0.05) {
      ctx.save();
      ctx.shadowColor = `rgba(255,${Math.round(150 - hotness * 100)},0,${hotness * 0.6})`;
      ctx.shadowBlur  = fuelW * 2;
      ctx.fillStyle   = `rgba(${rBase},${gBase},${bBase},0.01)`; // near-transparent fill, glow is the point
      for (let i = 0; i < nCh; i++) {
        const fx = c.x + 8 + i * chW + chGap;
        const fy = c.y + 8;
        const fh = c.h - 16;
    ...
    // Fuel channel fill pass
    for (let i = 0; i < nCh; i++) {
      const fx = c.x + 8 + i * chW + chGap;
      const fy = c.y + 8;
      const fh = c.h - 16;

      // Water channel cladding
      ctx.fillStyle = `rgba(30,90,170,0.5)`;
      ctx.fillRect(fx - 1, fy, fuelW + 2, fh);

      // Fuel pellet gradient (linear, position-dependent → created per channel)
  ...
  private _drawControlRods(ctx: CanvasRenderingContext2D, L: Layout, sim: ReactorState): void {
    const c         = L.core;
    const nRods     = 13;
    const insertion = sim.rodInsertion / 100;
    const chW       = (c.w - 16) / nRods;
    const rodW      = Math.max(2, chW * 0.35);
    const maxLen    = c.h - 8;
    const rodLen    = maxLen * insertion;

    for (let i = 0; i < nRods; i++) {
      const rx = c.x + 8 + i * chW + chW / 2 - rodW / 2;
      const ry = c.y;

      ctx.fillStyle = 'rgba(0,0,0,0.25)';
      ctx.fillRect(rx - 1, ry, rodW + 2, rodLen + 3);

      const g = ctx.createLinearGradient(rx, 0, rx + rodW, 0);
      g.addColorStop(0,   '#1a252f');
      g.addColorStop(0.4, '#2c3e50');
      g.addColorStop(1,   '#1a252f');
      ctx.fillStyle = g;
      ctx.fillRect(rx, ry, rodW, rodLen);

      if (rodLen > 4) {
        ctx.fillStyle = '#7f8c8d';
        ctx.fillRect(rx, ry + rodLen - 4, rodW, 4);
      }
      ...
      if (insertion < 0.99) {
        ...
        ctx.setLineDash([2, 3]);
        ctx.beginPath();
        ctx.moveTo(rx + rodW / 2, L.vessel.y);
        ctx.lineTo(rx + rodW / 2, ry);
        ctx.stroke();
        ctx.setLineDash([]);
      }
    }
    ...
    if (insertion < 0.99) {
      ...
      ctx.fillRect(c.x, L.vessel.y, c.w, 6);
      ctx.fillStyle = '#3d5166';
      ctx.fillRect(c.x, L.vessel.y, c.w, 2);
    }
  }
  ...
  private _drawParticles(ctx: CanvasRenderingContext2D, L: Layout, sim: ReactorState): void {
    ...
    for (const p of this._particles) {
      ...
      if (p.type === 'water') {
        if (p.y > L.vessel.y - 20 && p.y < L.vessel.y + L.vessel.h) {
          const alpha   = 0.4 + pumpN * 0.5;
      ...
      } else if (p.type === 'steam' && fluxN > 0.05) {
        ...
        ctx.fill();
      } else if (p.type === 'neutron' && fluxN > 0.02) {
        ...
        ctx.fill();
        // Halo (radialGradient here is acceptable — one per neutron,
        // neutron count is ≤60 and they don't change every frame)
  ...
  private _drawSteamSeparators(ctx: CanvasRenderingContext2D, L: Layout, sim: ReactorState): void {
    const fluxN      = sim.neutronFlux / 100;
    const steamAlpha = 0.5 + fluxN * 0.4;
    ...
    for (const sep of [L.steamSepL, L.steamSepR]) {
      ...
      ctx.fillStyle   = '#dde8f8';
      ctx.beginPath();
      ctx.arc(sep.cx, sep.cy, sep.r, 0, Math.PI * 2);
      ctx.fill();
      ctx.shadowBlur = 0;

      const sg = ctx.createRadialGradient(sep.cx, sep.cy - sep.r * 0.3, 2, sep.cx, sep.cy, sep.r);
      sg.addColorStop(0, `rgba(200,230,255,${steamAlpha})`);
      sg.addColorStop(1, `rgba(180,220,255,${steamAlpha * 0.5})`);
      ctx.fillStyle = sg;
      ctx.beginPath();
      ctx.arc(sep.cx, sep.cy, sep.r - 3, 0, Math.PI * 2);
      ctx.fill();
      ...
      ctx.fillRect(sep.cx - sep.r, sep.cy + sep.r - waterLevel * 2, sep.r * 2, waterLevel * 2);
      ctx.restore();
      ...
      ctx.lineWidth   = 1;
      ctx.beginPath();
      ctx.moveTo(sep.cx - sep.r + 3, sep.cy);
      ctx.lineTo(sep.cx + sep.r - 3, sep.cy);
      ctx.stroke();
    }
  }
  ...
  private _drawPump(ctx: CanvasRenderingContext2D, L: Layout, sim: ReactorState): void {
    ...
    for (let b = 0; b < 4; b++) {
      ...
      ctx.beginPath();
      ctx.ellipse(0, -p.r * 0.4, 2, p.r * 0.3, 0, 0, Math.PI * 2);
      ctx.fill();
    }
    ctx.restore();
    ...
    ctx.strokeStyle = pumpN > 0.1 ? '#1e3799' : '#5d6d7e';
    ctx.lineWidth   = 2;
    ctx.beginPath();
    ctx.arc(p.cx, p.cy, p.r, 0, Math.PI * 2);
    ctx.stroke();
  ...
  private _drawTurbine(ctx: CanvasRenderingContext2D, L: Layout, sim: ReactorState): void {
    const t      = L.turbine;
    const powerN = sim.powerOutput / 300;
    const turbOn = sim.turbineOn;
    ...
    const cx     = t.x + t.w / 2;
    const cy     = t.y + t.h / 2;
    const rOuter = t.w * 0.38;

    ctx.fillStyle = '#1a252f';
    ctx.beginPath();
    (ctx as any).roundRect(t.x, t.y, t.w, t.h, 8);
    ctx.fill();
    ...
    for (let i = 0; i < 6; i++) {
      ...
      ctx.fillStyle = turbOn && powerN > 0.1
        ? `rgba(74,158,255,${0.6 + powerN * 0.4})`
        : 'rgba(100,130,160,0.5)';
      ctx.beginPath();
      ctx.moveTo(0, -3);
      ctx.lineTo(rOuter * 0.7, -rOuter * 0.2);
      ctx.quadraticCurveTo(rOuter, 0, rOuter * 0.7, rOuter * 0.2);
      ctx.lineTo(0, 3);
      ctx.closePath();
      ctx.fill();
    }
    ...
    ctx.fillStyle = turbOn ? '#4a9eff' : '#5d6d7e';
    ctx.beginPath();
    ctx.arc(cx, cy, 5, 0, Math.PI * 2);
    ctx.fill();

    ctx.strokeStyle = turbOn ? '#2563eb' : '#3d5166';
    ctx.lineWidth   = 1.5;
    ctx.beginPath();
    (ctx as any).roundRect(t.x, t.y, t.w, t.h, 8);
    ctx.stroke();
  ...
  private _drawCoreGlow(ctx: CanvasRenderingContext2D, L: Layout, sim: ReactorState): void {
    ...
    ctx.fillStyle = glow;
    ctx.beginPath();
    ctx.arc(cx, cy, c.w * 0.9, 0, Math.PI * 2);
    ctx.fill();
  }
  ...
  private _drawLabels(ctx: CanvasRenderingContext2D, L: Layout, sim: ReactorState): void {
    ...
    for (const lb of labels) {
      ...
      ctx.font      = lb.bold ? '700 10px "Segoe UI"' : '10px "Segoe UI"';
    ...
    ctx.textAlign  = 'right';
    ctx.fillStyle  = sim.neutronFlux > 5 ? '#f39c12' : '#7f8c8d';
